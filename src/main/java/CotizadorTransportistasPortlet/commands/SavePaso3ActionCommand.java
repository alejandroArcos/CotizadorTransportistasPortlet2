...
import com.tokio.cotizadorModular.Bean.PersonasBloqueadasResponse;
import com.tokio.cotizadorModular.Bean.Registro;
import com.tokio.cotizadorModular.Bean.RetroactividadRequest;
import com.tokio.cotizadorModular.Bean.UmbralVoBo;
import com.tokio.cotizadorModular.Bean.ValidaResponse;
...
import CotizadorTransportistasPortlet.bean.LogVoBo;
import CotizadorTransportistasPortlet.bean.ModoAuxiliar;
...
public class SavePaso3ActionCommand extends BaseMVCActionCommand {
	...
	protected void doProcessAction(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		...
		actionResponse.setRenderParameter("jspPage", "/pantallas/paso4.jsp");
	}
	...
	private void fQuienEsQuien(ActionRequest actionRequest){
		...
		try {
			...
			WebResource webResource = client.resource("https://qeq.com.mx/datos/qws/access")
					.queryParam("var1", CotizadorTransportistasPortletKeys.QQ_USUARIO)
					.queryParam("var2", CotizadorTransportistasPortletKeys.QQ_CONTRASEÑA);
			...
			if (Validator.isNull(cookieId)) {
				...
				if (isMoral){
					...
					WebResource	razMor = client.resource(CotizadorTransportistasPortletKeys.QQ_URL_MORAL)
							.queryParam("razonsoc",
									HtmlUtil.escape( responseEmision.getDatosCliente().getNombre().toLowerCase()));					
					
					WebResource	rfcMor = client.resource(CotizadorTransportistasPortletKeys.QQ_URL_MORAL)
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());

					WebResource	nom_den_rfc = client.resource(CotizadorTransportistasPortletKeys.QQ_URL_MORAL)
							.queryParam("razonsoc",
									HtmlUtil.escape( 
					...
					WebResource nomCom = client.resource(CotizadorTransportistasPortletKeys.QQ_URL_FISICA)
					...
					WebResource nomRFC = client.resource(CotizadorTransportistasPortletKeys.QQ_URL_FISICA)
							.queryParam("nombre", HtmlUtil.escape(responseEmision.getDatosCliente().getNombre().toLowerCase()))
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());					

					WebResource apPRFC = client.resource(CotizadorTransportistasPortletKeys.QQ_URL_FISICA)
							.queryParam("paterno", HtmlUtil.escape(responseEmision.getDatosCliente().getAppPaterno().toLowerCase()))
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());					

					WebResource apMRFC = client.resource(CotizadorTransportistasPortletKeys.QQ_URL_FISICA)
							.queryParam("materno", HtmlUtil.escape(responseEmision.getDatosCliente().getAppMaterno().toLowerCase()))
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());
